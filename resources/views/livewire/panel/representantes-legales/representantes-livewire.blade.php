<div wire:init="loadData">
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">{{ __('Representantes Legales') }}
                        @can('representante.legal.store')
                    ...
                    <div class="row col-12 mb-4">
                        <div class="col-lg-4 col-md-4 col-12 mb-2">
                            <span>{{ __('Buscar representante legal') }}</span>
                            <input type="search" class="form-control @error('search') is-invalid @enderror"
                                placeholder="{{ __('Buscar por nombre, apellido y otros') }}" wire:model="search">
                            @error('search')
                        ...
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    ...
                                    @can(['representante.legal.edit', 'representante.legal.delete'])
                                        <th scope="col">{{ __('Accion') }}</th>
                                    @endcan
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($this->Representantes as $item)
                                    <tr>
                                        <th scope="row">#{{ $item->id }}</th>
                                        <td>{{ $item->name . ' ' . $item->last_name }}</td>
                                        <td>{{ $item->documento->name . ' ' . $item->document }}</td>
                                        <td>{{ $item->email }}</td>
                                        <td>{{ $item->phone }}</td>
                                        <td>
                                            @if ($item->default == 1)
                                                <span class="badge bg-success"><i class="fas fa-user-check"></i></span>
                                            @else
                                                <span class="badge bg-secondary text-white-50"><i
                                                ...
                                                <div class="dropdown dropstart">
                                                    ...
                                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                        @can('representante.legal.edit')
                                                            ...
                                                            @if ($item->default != 1)
                                                                <li><button class="dropdown-item"
                                                                        wire:click="default({{ $item->id }})">
                                                                        <i class="fas fa-user-check"></i> Hacer por
                                                                        defecto</button>
                                                                </li>
                                                            @endif
                                                        @endcan

                                                        @can('representante.legal.delete')
                                                            <li><button class="dropdown-item"
    ...
    @include('panel.representantes.modal.create')
    @include('panel.representantes.modal.edit')
    ...
    <script>
        ...
        window.addEventListener('changeDefaul', event => {
            Swal.fire({
                icon: 'question',
                title: "¿Estas seguro?",
                text: "Actualmente el representante por defecto es " + event.detail.name,
                showCancelButton: true,
            }).then((result) => {
                if (result.value) {
                    window.livewire.emit('changeDefault')
                    let timerInterval
                    Swal.fire({
                        icon: 'success',
                        title: '¡Procesando! ',
        ...
        window.addEventListener('changeDefault', event => {
            Swal.fire({
                icon: 'success',
                title: '¡Procesando! ',
                text: 'Espera un momento, pronto estará disponible',
